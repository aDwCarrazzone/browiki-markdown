---
title: "Loira do Banheiro"
source: https://browiki.org/wiki/Loira_do_Banheiro
namespace: 0
revision: 27978
retrieved: 2026-06-23T21:03:27Z
categories:
  - "Quests de experiência"
  - "Quest"
  - "Acesso a Calabouços"
---

# Loira do Banheiro

| NQ ico Loira do Banheiro | |
| --- | --- |
| Boatos de que o fantasma de uma mulher morta está rondando os banheiros de um dos pontos turísticos mais famosos da cidade! Você tem coragem de fazer o ritual de invocação? | |
| Requisitos mínimos | |
| **Nv. de base** | 40 |
| **Tipo** | Normal |
| **[Itens](Itens.md "Itens")** | 11515.png 1 [Coco](http://www.divine-pride.net/database/item/11515/) |
| Premiação | |
| **[EXP base](Experiência de Base.md "Experiência de Base")** | 90.000 |
| **Recompensa** | Acesso às [Profundezas das Cataratas](http://www.divine-pride.net/database/map/bra_dun01/) |

1. Fale com 1 das 4 crianças (brasilis 183,245)

   :   ![A Loira do Banheiro01](images/b/b1/A_Loira_do_Banheiro01.png)
2. Para entrar no banheiro, entregue o [Coco](http://www.divine-pride.net/database/item/11515/) ao Curador (bra\_in01 9,181)

   :   ![A Loira do Banheiro02](images/b/b0/A_Loira_do_Banheiro02.png)

   - O [Coco](http://www.divine-pride.net/database/item/11515/) é comprado no Feirante (brasilis 221,128)
   - Nesta parte da quest, sempre que você quiser entrar no banheiro, será necessário dar outro [Coco](http://www.divine-pride.net/database/item/11515/) ao Curador.
3. No banheiro, clique nos [NPCs](NPC.md "NPC") em ordem:

   :   ![Brasilisbanheiro](images/b/bf/Brasilisbanheiro.png)

   1. **Porta:** Selecione *"Mexer"* e escreva a frase abaixo:
      - `Mamãe, a porta não abre!`
      - Na sequência, escolha *"Girar a Chave"*
      - Digite `7`
   2. **Sanitário:** Selecione *"Mexer"* e escreva a frase abaixo:
      - `Mamãe, a água já está transbordando!!`
      - Na sequência, escolha *"Tirar a água"*
      - Digite `3`
   3. **Torneira:** Selecione *"Mexer"* e escreva a frase abaixo:
      - `Mamãe, a enchente começou!!`
      - Na sequência, escolha *"Abrir a torneira"*
      - Digite `1`
   4. **Tapete:** Selecione *"Mexer"* e escreva a frase abaixo:
      - `Mamãe, onde estão meus amigos?`
      - Na sequência, escolha *"Virar o Tapete"*
      - Digite `7`
   5. **Espelho:** Selecione *"Mexer"* e escreva a frase abaixo:
      - `Mamãe, onde você está, mamãe?`
      - O fantasma da loira do banheiro aparecerá.
   - Tire as ataduras da Loira do Banheiro, isso fará com que vocês lancem os dados.
   - Se o número do dado da Loira for menor que o seu, você teleporta para um cano de esgoto.
   - Se for maior, o fantasma de expulsa do banheiro e você terá que repetir desde o passo *2.*
4. No cano de esgoto, siga reto e clique no *Vácuo Aberto*.

   :   ![A Loira do Banheiro04](images/b/b6/A_Loira_do_Banheiro04.png)
5. Para entrar no calabouço, existem duas formas:
   - Seguir aos campos de Brasilis e clicar no Cano (bra\_fild01 323,136)

   :   ![A Loira do Banheiro05](images/e/e1/A_Loira_do_Banheiro05.png)

   - Ou você pode voltar ao banheiro, clicar no Vaso Sanitário e escolher "*Tirar a água*".
   - Para sair do calabouço, clique no Cano (bra\_dun01 87,46)

   :   ![A Loira do Banheiro06](images/a/a6/A_Loira_do_Banheiro06.png)
