# Humor dos Jogadores

# Humor dos Jogadores

| ![NQ ico](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/8/82/NQ_ico.png) Humor dos Jogadores | |
| --- | --- |
| Após uma sequência de derrotas, será difícil encontrar jogadores motivados a continuar participando dos Jogos Felinos. | |
| Requisitos mínimos | |
| **Nv. de base** | 60 |
| **Tipo** | Normal |
| Premiação | |
| **[Itens](../../Itens/Itens.md "Itens")** | ![6422.png](https://static.divine-pride.net/images/items/item/6422.png) 100 [Moedas de Classe E](http://www.divine-pride.net/database/item/6422/)  ![2872.png](https://static.divine-pride.net/images/items/item/2872.png) 1 [Certificado de Honra G](http://www.divine-pride.net/database/item/2872/) |
| **Recompensa** | Acesso aos [Jogos Felinos](Jogos%20Felinos.md "Jogos Felinos") |

1. Fale com o Diretor dos Jogos (mal\_in01 31,210)

   :   ![Catgamer01](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/a/a5/Catgamer01.png)
2. Insista em conversar com o Gato Estranho (malangdo 127,111).

   :   ![Catgamer02](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/4/4f/Catgamer02.png)

   - Clique no [NPC](../../Redirecionamentos/NPC.md "NPC") diversas vezes até você conseguir dar continuidade na missão.
3. Volte ao Diretor e conte a verdade (mal\_in01 31,210)
   - Você deve aguardar 3 minutos para falar com ele novamente.
4. Encontre 4 [NPCs](../../Redirecionamentos/NPC.md "NPC") escondidos em [Malangdo](../../Lugares/Malangdo.md "Malangdo"). Cada 1 só aparece se os outros 3 estiverem desaparecidos, ou seja, se um [NPC](../../Redirecionamentos/NPC.md "NPC") não estiver no local indicado, procure pelos outros 3 até ter falado com todos.
   1. Diga que você é um Redador de Revista para Eryu (malangdo 161,197)

      :   ![Catgamer03](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/4/4b/Catgamer03.png)
   2. Dê notícias de investimentos a Eff (malangdo 181,119)

      :   ![Catgamer05](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/f/f8/Catgamer05.png)
   3. Fale sobre a 'Latas para Gatinhos' para Ketchup (malangdo 244,144)

      :   ![Catgamer06](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/7/7d/Catgamer06.png)
   4. Mencione a empresa 'Enlatados Bum' para Stew (malangdo 230,197)

      :   ![Catgamer04](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/c/ca/Catgamer04.png)

      - Se Stew não estiver aparecendo, visite outros gatos.
5. Volte ao Diretor (mal\_in01 31,210)
   - Você deve aguardar 3 minutos para falar com ele novamente.
6. Reencontre o Gato Estranho (malangdo 127,111)
   - Selecione a opção que tem Fogo.
7. Volte ao Diretor dos Jogos. (mal\_in01 31,210)
   - Você deve aguardar 20 horas para receber 100 [Moedas de Classe E](http://www.divine-pride.net/database/item/6422/).
8. Fale com o Diretor dos Jogos novamente.
   - Você deve aguardar 1 hora para receber 1 [Certificado de Honra G](http://www.divine-pride.net/database/item/2872/).

---
*Categorias: Quest*  
*Importado da [bROWiki](https://browiki.org/wiki/Humor_dos_Jogadores) — revisão 39019*
