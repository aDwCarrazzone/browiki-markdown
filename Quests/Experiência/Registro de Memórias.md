# Registro de Memórias

# Registro de Memórias

| ![EP ico](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/3/3f/EP_ico.png) Registro de Memórias | |
| --- | --- |
| Desvende os relatos dos últimos sobreviventes que gravaram os registros de memórias! | |
| Requisitos mínimos | |
| **Nv. de base** | 140 |
| **Tipo** | Episódio |
| **Quests anteriores** | [Fantasmagórica](../../Calabou%C3%A7os%20e%20Inst%C3%A2ncias/Fantasmag%C3%B3rica.md "Fantasmagórica") |
| Premiação | |
| **[EXP base](../../Redirecionamentos/Experi%C3%AAncia%20de%20Base.md "Experiência de Base")** | 6.000.000 |
| **[EXP classe](../../Redirecionamentos/Experi%C3%AAncia%20de%20Classe.md "Experiência de Classe")** | 4.000.000 |

1. Selecione a opção "*Conversar*" com Ian Atnad (verus04 144,193)

   :   ![IanAtnad](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/4/44/IanAtnad.png)
2. Dirija-se ao [Instituto WISH de Pesquisa](http://www.divine-pride.net/database/map/verus02/) e colete 1 [Registro de Memórias do Instituto](http://www.divine-pride.net/database/item/6757/) dos Papéis Espalhados.

   :   ![Papeis Espalhados](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/9/92/Papeis_Espalhados.png)
3. Retorne a Ian Atnad.
4. Fale com a Capitã Arquien no segundo andar do [Grupo do Éden](../Gerais/Grupo%20do%20%C3%89den.md "Grupo do Éden") (moc\_para01 133,170).

   :   ![Arquien](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/a/a8/Arquien.png)

   - Digite:
     - `Reprodutor de Registro`
   - Converse mais uma vez com a Capitã Arquien
5. Colete mais 5 [Registros de Memórias do Instituto](http://www.divine-pride.net/database/item/6757/) e traga-os para a Capitã Arquien (verus04 170,148)

   :   ![NileAtnad](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/1/1f/NileAtnad.png)
6. Clique no Reprodutor ao lado da Capitã.

   :   ![Reprodutor](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/b/b6/Reprodutor.png)

   - Ouça o conteúdo do registro e fale com a Capitã Arquien em seguida.
   - Você receberá 750.000 de [EXP](../../Redirecionamentos/EXP.md "EXP") de base e 500.000 de [EXP](../../Redirecionamentos/EXP.md "EXP") de classe.
7. Conte a Ian Atnad sobre os registros.
8. Dirija-se ao [Laboratório OPTATIO de Experimentos](http://www.divine-pride.net/database/map/verus01/) e acesse o abrigo falando com Verity (verus01 123,181)

   :   ![Verity](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/1/1a/Verity.png)
9. Colete 5 [Registros de Memórias do Abrigo](http://www.divine-pride.net/database/item/6824/) clicando nos [NPCs](../../Redirecionamentos/NPC.md "NPC") espalhados no mapa.

   :   ![NPCsabrigo1](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/b/b3/NPCsabrigo1.png)
10. Volte a falar com a Capitã Arquien em [Verus](../../Lugares/Verus.md "Verus").
    - Clique no Reprodutor ao lado dela para ouvir o registro.
    - Você receberá 1 [Registro do 1º Dia](http://www.divine-pride.net/database/item/22691/). NÃO utilize este item.
    - Mostre o registro para a Capitã.
    - Você receberá 750.000 de [EXP](../../Redirecionamentos/EXP.md "EXP") de base e 500.000 de [EXP](../../Redirecionamentos/EXP.md "EXP") de classe.
11. Fale com Ian Atnad.
    - Selecione:
      - Sobre os Registros de Memória.
    - Agora você pode ir usando os Registros do 1º, 30º, 32º, 40º e 50º dia para fazer as missões diárias do [Sobrevivendo no Abrigo](Sobrevivendo%20no%20Abrigo.md "Sobrevivendo no Abrigo").
    - Você liberou as missões diárias [Registro de Memórias do Abrigo e do Instituto](Aventuras%20em%20Verus.md "Aventuras em Verus").
    - Para finalizar a [quest](../../Redirecionamentos/Quest.md "Quest"), você deverá repetir os passos **5** e **9** por 5 dias.
    - Não se preocupe com a numeração dos dias do pedaço de registro, o que contará para finalizar a missão é a quantidade de vezes que você entregou os registros de memórias para Arquien.
    - No 5º dia, selecione a opção "*Conversar*" com a Capitã Arquien e, em seguida, fale com Ian Atnad para que o Secretário Chefe da Corporação Rekenber apareça.
12. Dirija-se até a Sede da Corporação Rekenber e fale com o Guarda da Rekenber (lhz\_in01 276,234)

    :   ![GuardinhaRK](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/8/8f/GuardinhaRK.png)
13. Converse com todos os presentes na reunião para terminar a história.

    :   ![PresidenteRekenber](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/7/72/PresidenteRekenber.png)

    - Você receberá 4.500.000 de [EXP](../../Redirecionamentos/EXP.md "EXP") de base e 3.000.000 de [EXP](../../Redirecionamentos/EXP.md "EXP") de classe.

![Saga4](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/8/84/Saga4.png)

| Parte I | | Parte II | | Parte III | |
| --- | --- | --- | --- | --- | --- |
| [Fantasmagórica](../../Calabou%C3%A7os%20e%20Inst%C3%A2ncias/Fantasmag%C3%B3rica.md "Fantasmagórica") | ![SetaVermelha](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/0/0f/SetaVermelha.png) | [Investigando o Passado](../../Calabou%C3%A7os%20e%20Inst%C3%A2ncias/Investigando%20o%20Passado.md "Investigando o Passado") | ![SetaVermelha](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/0/0f/SetaVermelha.png) | [Sala Final](../../Calabou%C3%A7os%20e%20Inst%C3%A2ncias/Sala%20Final.md "Sala Final") |
| [Laboratório Central](../../Calabou%C3%A7os%20e%20Inst%C3%A2ncias/Laborat%C3%B3rio%20Central.md "Laboratório Central") | | |
| Registro de Memórias | ![SetaVermelha](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/0/0f/SetaVermelha.png) | [Sobrevivendo no Abrigo](Sobrevivendo%20no%20Abrigo.md "Sobrevivendo no Abrigo") |

---
*Categorias: Quests de experiência, Quest*  
*Importado da [bROWiki](https://browiki.org/wiki/Registro_de_Mem%C3%B3rias) — revisão 39902*
