# Vila Cautelosa

# Vila Cautelosa

| ![NQ ico](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/8/82/NQ_ico.png) Vila Cautelosa | |
| --- | --- |
| Os nativos da ilha ainda estranham os estrangeiros na cidade. É melhor você causar uma boa primeira impressão! | |
| Requisitos mínimos | |
| **Nv. de base** | 100 |
| **Tipo** | Normal |
| **[Itens](../Itens/Itens.md "Itens")** | ![523.png](https://static.divine-pride.net/images/items/item/523.png) 6 [Água Benta](http://www.divine-pride.net/database/item/523/)  ![717.png](https://static.divine-pride.net/images/items/item/717.png) 6 [Gema Azul](http://www.divine-pride.net/database/item/717/) |
| Premiação | |
| **[EXP base](../Sem%20categoria/Experi%C3%AAncia%20de%20Base.md "Experiência de Base")** | 200.000 |
| **[EXP classe](../Sem%20categoria/Experi%C3%AAncia%20de%20Classe.md "Experiência de Classe")** | 200.000 |
| **Recompensa** | Libera os [serviços](../Informa%C3%A7%C3%B5es%20b%C3%A1sicas/Servi%C3%A7os.md "Serviços") da cidade e a missão [Aventuras em Malaya](Aventuras%20em%20Malaya.md "Aventuras em Malaya") |

1. Fale com o Guarda Rondel (malaya 266,76)

   :   ![Vila Cautelosax](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/8/82/Vila_Cautelosax.png)
2. Encontre Phong, o Líder Mumbaki (malaya 185,358). Qualquer opção selecionada não altera a missão.

   :   ![Vila Cautelosa2](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/e/ee/Vila_Cautelosa2.png)
3. Converse com o Ancião Nardo (malaya 181,353)

   :   ![Malaya1](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/9/9f/Malaya1.png)
4. Fale com Ferreiro Pandoi (malaya 119,217)

   :   ![Malaya2](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/0/01/Malaya2.png)
5. Localize Romel (malaya 224,267)

   :   ![Malaya3](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/c/cd/Malaya3.png)
6. Dialogue com Talah (malaya 223,267)

   :   ![Malaya4](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/9/9f/Malaya4.png)
7. Apresente-se ao Colecionador Woeon (malaya 295,171)

   :   ![Malaya5](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/1/17/Malaya5.png)
8. Explique o que aconteceu a Phong (malaya 185,358). Em seguida, sugira [Água Benta](http://www.divine-pride.net/database/item/523/) e [Gema Azul](http://www.divine-pride.net/database/item/717/).
9. Encontre o Marinheiro Pedro (malaya 326,68)

   :   ![Malaya6](https://cdn.jsdelivr.net/gh/aDwCarrazzone/browiki-images@main/2/2c/Malaya6.png)
10. Colete os itens e entregue-os para cada [NPC](../Sem%20categoria/NPC.md "NPC") já mencionados acima:
    - Ancião Nardo (malaya 181,353)
    - Ferreiro Pandoi (malaya 119,217)
    - Romel (malaya 224,267)
    - Talah (malaya 223,267)
    - Colecionador Woeon (malaya 295,171)
    - Marinheiro Pedro (malaya 326,68)
11. Retorne a Phong (malaya 185,358)
    - Você recebe sua recompensa em [experiência](../Sem%20categoria/Experi%C3%AAncia.md "Experiência") e tem acesso aos prédios da cidade, tais como a loja de ferramentas, a [estalagem](../Sem%20categoria/Estalagem.md "Estalagem") e usar o *jeepney* para se locomover na cidade.
